...
public class VocabularyEntityTest extends TestCase {
	...
	public void testUnions() {
		...
		assertTrue(this.entityProps.containsKey("union_1"));
		assertTrue(this.entityProps.containsKey("union3"));
		
		System.out.println(this.entityProps.get("union1").isEmpty());
		assertFalse(this.entityProps.get("union_1").isEmpty());
		assertFalse(this.entityProps.get("union3").isEmpty());

		List<String> attributeProps = this.entityProps.get("union_1");
		for (Iterator<String> it = attributeProps.iterator(); it.hasNext();) {
