...
public class LOCCountPerEntity {
	 ...
	 * @param sourceCode
	 */
	public LOCCountPerEntity(ASTNode type, List<IASTComment> comments, String sourceCode, List<ASTNode> listDeclarations, int headersLOC) {
		this.allComments = comments;
		this.inner = 0;
		this.loc = locCount(type, getScopeComments(type, comments), sourceCode) - headersLOC;
		
		for (ASTNode t :  listDeclarations) {

			inner += locCount(t, getScopeComments(t, comments), sourceCode);
		}
	...
	/**
	 * Creates an array of indices that delimit the begin and end of each inner
	 * class.
	 * 
	 * @param type
	 *            The given type which provides the necessary inner entities
	 *            information.
	 * @return
	 */
