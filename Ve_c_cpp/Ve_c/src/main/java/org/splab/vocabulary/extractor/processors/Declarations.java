...
public class Declarations {
	...
	private static void extractSimpleDeclaration(CASTSimpleDeclaration declaration, boolean scope) {
		...
		for (IASTDeclarator declarator : simpleDeclarations) {
			...
			if (scope == true) {
				...
				if (declarator instanceof CASTFunctionDeclarator)
					FileVocabularyManager.insertGlobalPrttp(name, access, storage);
				else {
					FileVocabularyManager.insertGlobalVariable(name, access, storage);
				}
			}
	...
	private static void extractDesignatedInitializer(CASTDesignatedInitializer designated) {
	...
	private static void extractDesignators(ICASTDesignator designator) {
	...
	private static void extractArrayRangeDesignator(CASTArrayRangeDesignator arrayRangeDesignator) {
		...
		if (arrayRangeDesignator.getRangeCeiling() instanceof CASTArrayDesignator)
			extractArrayRangeDesignator((CASTArrayRangeDesignator) arrayRangeDesignator.getRangeCeiling());

		if (arrayRangeDesignator.getRangeFloor() instanceof CASTArrayDesignator)
	...
	public static DeclarationsType declarations(CASTSimpleDeclaration simpleDeclaration,
		...
		if (declarations instanceof CASTEnumerationSpecifier) {
			...
			EnumProcessor enumProcessor = new EnumProcessor(enumeration, scope, true);
		...
		if (declarations instanceof CASTTypedefNameSpecifier) {
			...
			valor = DeclarationsType.VARIABLE;
		}
