...
public class AnnotationCounter {
	...
	private int[][] limits;
	private char[] source;
	private List<IASTComment> comments;
	 ...
	 */
	public AnnotationCounter(ASTNode type, int[][] limits, List<IASTComment> comments, String sourceCode) {
	...
	private boolean isInComment(int index) {
		for(IASTComment c : comments) {
			...
			int begin = comment.getOffset(), end = begin + comment.getLength();
