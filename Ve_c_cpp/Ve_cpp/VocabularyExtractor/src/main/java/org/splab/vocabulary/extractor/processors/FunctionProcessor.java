...
public class FunctionProcessor {
	...
	public FunctionProcessor(CPPASTFunctionDefinition functionDefinition, EntityType entityType, int indentationLevel) {
		...
		if (this.body == null) {
			vxlFragment = new StringBuffer(VxlManager.funtionPrototype(StringProcessor.processString(name), access,
					storage, indentationLevel));
			return;
		}
		...
		vxlFragment = new StringBuffer(VxlManager.startFuntion(StringProcessor.processString(this.name), this.access,
				this.storage, locKeeper.getLOC() + locKeeper.getInnerEntitiesLOC(), indentationLevel));
		vxlFragment.append(
		...
		if (functionDefinition.getDeclarator() instanceof CPPASTFunctionDeclarator) {
			...
			for (IASTParameterDeclaration parameter : parameters) {
				if (!parameter.getDeclarator().getName().toString().equals("")) {
					...
					VxlManager.parameter(vxlFragment, StringProcessor.processString(parametroName), (indentationLevel));
	...
	public void storeInternVocabularyField(Map<String, Integer> fields, Map<String, String> fieldAccess,
		...
		while (it_fields.hasNext()) {
			...
			if (fields.get(identifier) > 0) {
				...
				vxlFragment.append(VxlManager.field(StringProcessor.processString(identifier), access, storage, visib,
						count, indentationLevel));
			}
		}
	}
