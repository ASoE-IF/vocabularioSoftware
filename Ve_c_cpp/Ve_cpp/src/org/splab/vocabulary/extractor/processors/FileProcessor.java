...
public class FileProcessor{
	...
	private StringBuffer variableVXL;
	...
	public FileProcessor(IASTTranslationUnit translationUnit, String fileName, int headerLines) {
		...
		// mantem as informações dos protótipos de funções
		prttpStorage = new TreeMap<String, String>();
		prttpAccess = new TreeMap<String, String>();
		
		// mantem as informações dos leterais
		...
		for (IASTDeclaration nodeDeclaration : declarations) {
			...
			if (nodeDeclaration instanceof IASTFunctionDefinition) {
				FunctionProcessor functions = new FunctionProcessor((CPPASTFunctionDefinition) nodeDeclaration, EntityType.FUNCTION);
			...
			if (nodeDeclaration instanceof IASTSimpleDeclaration) {
				DeclarationsType valor = Declarations.declarations((CPPASTSimpleDeclaration) nodeDeclaration, null,
						false);
				...
				else if (valor == DeclarationsType.CLASS)
					classVXL.append(Declarations.getVxlFragment());

			}
			
			if(nodeDeclaration instanceof ICPPASTTemplateDeclaration){
				
				CPPASTTemplateDeclaration template = (CPPASTTemplateDeclaration) nodeDeclaration;
				System.out.println(nodeDeclaration.getRawSignature());
				System.out.println(nodeDeclaration.getClass());
				
				System.err.println(template.getDeclaration().getClass());
				System.err.println(template.getDeclaration().getRawSignature());
				
				CPPASTSimpleTypeTemplateParameter dd = (CPPASTSimpleTypeTemplateParameter)template.getTemplateParameters()[0];
		...
		vxlFragment.append(variableVXL);
