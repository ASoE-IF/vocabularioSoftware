...
public class StructProcessor {
	 ...
	 */
	public StructProcessor(CPPASTCompositeTypeSpecifier compositeType, VocabularyManager vocabularyManager,
		...
		vxlFragment.append(
				VxlManager.startStruct(compositeType.getName().toString(), locKeeper.getLOC(), indentationLevel));
		...
		for (IASTDeclaration membersSimple : members) {
			if (membersSimple instanceof CPPASTSimpleDeclaration) {
				...
				if (structMembers.length != 0) {
					vxlFragment.append(VxlManager.field(structMembers[0].getName().toString(), indentationLevel + 1));
				}

				if (simpleMembers.getDeclSpecifier() instanceof CPPASTCompositeTypeSpecifier) {
					declProcessor.extractDeclaration(simpleMembers, indentationLevel);
				}
				if (simpleMembers.getDeclSpecifier() instanceof CPPASTEnumerationSpecifier) {
					declProcessor.extractDeclaration(simpleMembers, indentationLevel);
				}
			}
		}
