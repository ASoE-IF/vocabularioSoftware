...
public class LOCManager {
	...
	public static int totalLOC; // total de linhas de código desconciderando
								// readers
	public static int totalHeader;// total de linhas de cabeçalhos
	public static int totalInnerEntitiesLines;// total de linhas em entidades
												// internas
	public static int totalCodeLines; // total de linhas de código

	public static void reset() {
		entitiesLOCContent = new StringBuffer(
				",Entity's Name,LOC Count,Header Count,Inner Entities Lines,Total,Entity Type");
		compilationUnitLOCContent = new StringBuffer("Name,LOC Count");
		totalLOC = totalHeader = totalInnerEntitiesLines = totalCodeLines = 0;
	}
	...
	public static void appendEntityLOCData(String typeName, EntityLOCKeeper entity, EntityType entityType) {
		...
		int total = entity.getLOC() + entity.getInnerEntitiesLOC() + entity.getHeaderLOC();
		entitiesLOCContent.append("\n," + typeName + "," + entity.getLOC() + "," + entity.getHeaderLOC() + ","
				+ entity.getInnerEntitiesLOC() + "," + total + "," + entityType.name());
	...
	public static void appendEntityLOCData2(String typeName, EntityLOCKeeper entity, EntityType entityType) {

		int total = entity.getLOC() + entity.getInnerEntitiesLOC() + entity.getHeaderLOC();
		appendEntityLOCData(typeName, entity, entityType);
	...
	public static void save(String fileName) {
		...
		entitiesLOCContent.append(
				String.format("\nTotal,,%d,%d,%d,%d", totalLOC, totalHeader, totalInnerEntitiesLines, totalCodeLines));

		try {
			PrintStream file = new PrintStream(new FileOutputStream(fileName, false));
			file.print(locParameters.contains(LOCParameters.ONLY_PHYSICAL_LINES) ? compilationUnitLOCContent.toString()
					: entitiesLOCContent.toString());
			file.close();
		} catch (IOException e) {
			e.printStackTrace();
		}
