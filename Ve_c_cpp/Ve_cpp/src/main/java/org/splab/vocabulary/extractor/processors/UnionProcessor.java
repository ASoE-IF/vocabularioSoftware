...
public class UnionProcessor {
	 ...
	 */
	public UnionProcessor(CPPASTCompositeTypeSpecifier compositeType, VocabularyManager vocabularyManager,
		...
		vxlFragment.append(
				VxlManager.startUnion(compositeType.getName().toString(), locKeeper.getLOC(), indentationLevel));
		...
		for (IASTDeclaration membersSimple : members) {
			if (membersSimple instanceof CPPASTSimpleDeclaration) {
				...
				if (unionMembers.length != 0) {
					vxlFragment.append(VxlManager.field(unionMembers[0].getName().toString(), indentationLevel + 1));
				}
