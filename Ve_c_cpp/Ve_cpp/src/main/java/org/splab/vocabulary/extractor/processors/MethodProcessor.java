...
public class MethodProcessor extends FunctionProcessor {
	...
	public MethodProcessor(CPPMethod method, EntityType entityType, int indentationLevel) {
		...
		vxlFragment = new StringBuffer(VxlManager.startMethod(this.name, this.access, this.storage, this.modificador,
				this.visibilidade, isVirtual, locKeeper.getLOC() + locKeeper.getInnerEntitiesLOC(), indentationLevel));
		vxlFragment.append(
	...
	public MethodProcessor(CPPMethod method, int indentationLevel) {
		...
		if (method.isPureVirtual())
			vxlFragment = new StringBuffer(
					VxlManager.methodPureVirtual(name, access, storage, modificador, visibilidade, indentationLevel));
