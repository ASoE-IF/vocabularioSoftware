...
public abstract class VXLIterator implements Iterator<ContainerEntity> {
	...
	protected CPPProject cppProject;
	protected EntityFilter entityFilter;
	protected Iterator<CPPFile> fileIterator;
	...
	public VXLIterator(Properties props) {
		...
		this.entityFilter.setProperties(includeFunction, includeClass, includeMethod, includeEnum, includeUnion,
				includeStruct, includeParameter, includeLocalVar, includeGlobalVar, includeFuncCall, includePrototype,
				includeLiterals, includeField, includeConstant, includePragma, includeDirective, includeError,
				includeMacro, includeComment);
	}
	...
	public static final VXLIterator createNewIterator(Properties props) {
		...
		if (props.containsKey(VXLReaderPropertyKeys.CONTAINER_TYPE)) {
			...
			} else if (containerTypeProp.equalsIgnoreCase(VXLReaderPropertyKeys.ContainerType.CLASS.toString())) {
				return new VXLClassesIterator(props);
			} else if (containerTypeProp.equalsIgnoreCase(VXLReaderPropertyKeys.ContainerType.METHOD.toString())) {
				return new VXLMethodsIterator(props);
			}
		}
	...
	/**
	 * Sets the c project used by this iterator to obtain the project entities
	 * 
	 * @param newCProject
	 *            the c project
	 */
	public void setCProject(CPPProject newCProject) {

		this.cppProject = newCProject;
		this.fileIterator = this.cppProject.getCPPFile().iterator();
	...
	// -----------------------------
	protected boolean includeFile = false, includeClass = false, includeMethod = false, includeFunction = false,
			includeEnum = false, includeUnion = false, includeStruct = false, includeParameter = false,
			includeLocalVar = false, includeGlobalVar = false, includeFuncCall = false, includePrototype = false,
			includeLiterals = false, includeField = false, includeConstant = false, includePragma = false,
			includeDirective = false, includeError = false, includeMacro = false, includeComment = false;
	...
	private void validateProperties(Properties props) {
		...
		this.includeClass = checkProperties(props, VXLReaderPropertyKeys.INCLUDE_CLASS);
		this.includeMethod = checkProperties(props, VXLReaderPropertyKeys.INCLUDE_METHOD);
