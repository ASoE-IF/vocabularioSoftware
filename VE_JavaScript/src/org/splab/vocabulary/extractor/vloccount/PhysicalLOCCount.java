...
public class PhysicalLOCCount {
	 ...
	 */
	public PhysicalLOCCount(List<Comment> comments, String sourceCode, int linesOfCode) {
	 ...
	 */
	public PhysicalLOCCount(List<Comment> comments, String sourceCode, int linesOfCode, int start, int finall) {
		this.loc = linesOfCode;
		this.sourceCode = sourceCode.toCharArray();
		this.loc -= allCommentsLOC(comments);
		// Usado para debug
		//System.out.println("===="+ allCommentsLOC(comments));
		this.start = start;
		this.finall = finall;
	 ...
	 * @return
	 */
	private int numbeOfEmptyLines(String sourceCode, List<Comment> comments) {
		int emptyLines = 0;
	 ...
	 * @return
	 */
	private int allCommentsLOC(List<Comment> commentList) {
	 ...
	 * @return
	 */
	private int commentLOC(Comment comment) {
		int begin = comment.getStartPosition();
		int end = begin + comment.getLength();
