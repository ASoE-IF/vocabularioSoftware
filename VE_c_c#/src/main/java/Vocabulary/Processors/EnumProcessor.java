...
public class EnumProcessor//processador de enums
{
	...
	public EnumProcessor(CASTEnumerationSpecifier enumDeclaration, boolean scope, boolean isInner, int headerLines)
	{
		...
		vxlFragment.append(VxlManager.startEnum(enumDeclaration.getName().toString(), "null", locKeeper.getLOC(), scope));
		...
		{
			vxlFragment.append(VxlManager.constant(constEnumerators.getName().toString(), "Não disponivel", null, scope));////grava no vxlFragment as consantes enum
		}
