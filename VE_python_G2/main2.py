...
#BUGS: (Ocorren quando sao encontradas determinadas sintaxes de python 3 )
...
# Tratando classes
listaClasses = []
arquivo = open('<Nome de arquivo a ser gerado>', 'w')
...
# This function extract Comments Multiple Lines (CML)
#Implementada por: Denilson

def extractCML(module):
   c = []
   ...
   try:
       ...
       for node in ast.walk(module):
            if isinstance(node, ast.Expr):
                    c.append(node.__dict__['value'].__dict__['s'])
   except KeyError:
       pass

    # Writing comments in the file vxl

   for t in range(len(c)):
       arquivo.write("     <cmtt name=\""+c[t].strip(' \n')+"\""" />\n")
...
s = []
allFiles = ""
myPath = "<Diretorio do arquivo do qual sera extraido o vocabulario>"

# Pula a primeira linha do File, a partir do qual sera feia a extracao
